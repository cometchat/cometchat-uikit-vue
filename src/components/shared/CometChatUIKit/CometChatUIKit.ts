...
export class CometChatUIKit {
  ...
  private static initiateAfterLogin() {
    if (CometChatUIKit.uiKitSettings != null) {
      let extensionList: ExtensionsDataSource[] =
        CometChatUIKit.uiKitSettings.getExtensions()?.length > 0
          ? CometChatUIKit.uiKitSettings.getExtensions()
          : this.defaultExtensions;
      this.enableCalling();
      if (extensionList?.length > 0) {
        (extensionList as ExtensionsDataSource[]).forEach(
          (extension: ExtensionsDataSource) => {
            extension?.enable();
          }
        );
      }
  ...
  static login(uid: string): Promise<CometChat.User> | undefined {
    if (!CometChatUIKit.checkAuthSettings()) return undefined;
    return new Promise((resolve, reject) => {
      CometChat.login(uid, CometChatUIKit.uiKitSettings!.authKey!)
  ...
  static loginWithAuthToken(
    authToken: string
  ): Promise<CometChat.User> | undefined {
    if (!CometChatUIKit.checkAuthSettings()) return undefined;
    return new Promise((resolve, reject) => {
