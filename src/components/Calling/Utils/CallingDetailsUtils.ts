...
export class CallingDetailsUtils {
  public static getDetailsTemplate(
    user?: CometChat.User,
    group?: CometChat.Group
  ): Array<CometChatDetailsTemplate> {
    return [this.getPrimaryDetailsTemplate(user, group)];
  }
  ...
  public static getPrimaryDetailsTemplate(
    user1?: CometChat.User,
    group1?: CometChat.Group
  ): CometChatDetailsTemplate {
    const template: CometChatDetailsTemplate = new CometChatDetailsTemplate({
      id: "callControls",
  ...
  private static getPrimaryOptions(
    ...
    const options: CometChatDetailsOption[] = [];
    options.push(
